...
trainall:
	export NUMEXPR_MAX_THREADS=24 && python main.py --mode=train --config-yml=$(CONFIG_YML) --dataset.train.src=$(LMDB_DATASET)train --dataset.val.src=$(LMDB_DATASET)val --model.max_num_elements=34 --optim.num_workers=8 --model.max_neighbors=14 --model.cutoff=8.0 --optim.batch_size=30 --optim.eval_batch_size=30 --amp
